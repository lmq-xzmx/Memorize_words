...
def create_test_users():
    ...
    for student_data in students:
        ...
        if created:
            ...
            user.save()
            print(f"创建学生用户: {user.real_name} ({user.username})")
        created_students.append(user)
    
    return created_students

def create_vocabulary_data():
    """创建词汇数据"""
    print("创建词汇数据...")
...
def create_learning_data(students, vocab_list, words):
    """创建学习数据"""
    print("创建学习数据...")
    ...
    for student in students:
        ...
        print(f"为用户 {student.real_name} 创建学习数据")
...
def main():
    ...
    try:
        # 创建用户
        students = create_test_users()
        
        # 创建词汇数据
        vocab_list, words = create_vocabulary_data()
        
        # 创建学习数据
        create_learning_data(students, vocab_list, words)
        
        print("\n测试数据创建完成！")
        print(f"创建了 {len(students)} 个学生用户")
        print(f"创建了 {len(words)} 个单词")
        print("\n可以访问以下管理页面查看数据:")
        print("- 用户管理: http://localhost:8001/admin/accounts/customuser/")
        print("- 词汇管理: http://localhost:8001/admin/words/word/")
        print("- 学习目标: http://localhost:8001/admin/vocabulary_manager/learninggoal/")
        print("- 学习计划: http://localhost:8001/admin/vocabulary_manager/learningplan/")
        print("- 学习会话: http://localhost:8001/admin/vocabulary_manager/studysession/")

...
class RoleApprovalAdmin(StandardRoleAdminMixin, admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """只显示管理员角色申请"""
        # 使用RoleService获取管理员角色代码，确保数据一致性
        admin_role_code = 'admin'  # 管理员角色的标准代码
        return super().get_queryset(request).filter(requested_role=admin_role_code)
...
class RoleUserGroupAdmin(StandardRoleAdminMixin, admin.ModelAdmin):
    ...
    def formfield_for_choice_field(self, db_field, request, **kwargs):
        """为角色字段使用标准角色选择器"""
        # 首先尝试使用StandardRoleAdminMixin的统一配置
    ...
    class Media:
        js = (
            'admin/js/dynamic_role_selector.js',
            'admin/js/role_user_group_admin.js',
        )
        css = {
            'all': ('admin/css/dynamic_role_selector.css',)
        }
...
# 注意：RoleLevel、RoleUser、UserExtension 已在专用的 role_hierarchy_admin 站点中注册
# 不在默认 admin 站点重复注册，避免冲突
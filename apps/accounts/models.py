...
class RoleLevel(models.Model):
    ...
    role_name = models.CharField('角色名称', max_length=50, help_text='角色显示名称')
    description = models.TextField('角色描述', blank=True, help_text='角色的详细描述')
    is_active = models.BooleanField('是否启用', default=True)
    sort_order = models.IntegerField('排序', default=0, help_text='显示顺序')
    created_at = models.DateTimeField('创建时间', auto_now_add=True)
    updated_at = models.DateTimeField('更新时间', auto_now=True)
    ...
    def __str__(self):
        return f"{self.role_name} ({self.role})"
    
    def get_user_count(self):
        """获取该角色下的用户数量"""
        return CustomUser.objects.filter(role=self.role, is_active=True).count()
    
    def get_extension_config_count(self):
        """获取该角色的增项配置数量"""
        return RoleExtension.objects.filter(role=self.role, is_active=True).count()
...
class RoleUser(models.Model):
    ...
    updated_at = models.DateTimeField('更新时间', auto_now=True)
    notes = models.TextField('备注', blank=True, help_text='用户在该角色下的备注信息')
    ...
    def __str__(self):
        return f"{self.role_level.role_name} - {self.user.username}"
    
    def get_extension_count(self):
        """获取该用户的增项数量"""
        return UserExtension.objects.filter(role_user=self, is_active=True).count()
    
    def sync_from_user(self):
        """从用户模型同步角色信息"""
        if self.user.role != self.role_level.role:
            # 角色不匹配，需要更新或删除
            correct_role_level = RoleLevel.objects.filter(role=self.user.role).first()
            if correct_role_level:
                self.role_level = correct_role_level
                self.save()
            else:
                # 如果没有对应的角色级别，标记为非活跃
...
class UserExtension(models.Model):
    ...
    role_extension = models.ForeignKey(
        RoleExtension, 
        on_delete=models.CASCADE, 
        verbose_name='增项配置'
    )
    field_value = models.TextField('字段值', blank=True, help_text='增项字段的值')
    is_active = models.BooleanField('是否启用', default=True)
    created_at = models.DateTimeField('创建时间', auto_now_add=True)
    updated_at = models.DateTimeField('更新时间', auto_now=True)
    created_by = models.ForeignKey(
    ...
    def __str__(self):
        return f"{self.role_user} - {self.role_extension.field_label}: {self.field_value}"
    ...
    def clean(self):
        ...
        # 验证角色一致性
        if self.role_user.role_level.role != self.role_extension.role:
            raise ValidationError({
                'role_extension': f'增项配置角色({self.role_extension.role})与用户角色({self.role_user.role_level.role})不匹配'
            })
    
    def get_display_value(self):
        """获取格式化的显示值"""
        if self.role_extension.field_type == 'choice' and self.role_extension.field_choices:
            try:
                choices = json.loads(self.role_extension.field_choices)
                choice_dict = dict(choices)
                return choice_dict.get(self.field_value, self.field_value)
...
# 信号处理器，用于自动同步数据
@receiver(post_save, sender=CustomUser)
def sync_role_user_on_user_save(sender, instance, created, **kwargs):
    """用户保存时自动同步RoleUser记录"""
    if not hasattr(instance, 'role') or not instance.role:
        return
    
    # 获取或创建对应的RoleLevel
    role_level, _ = RoleLevel.objects.get_or_create(
        role=instance.role,
        defaults={
            'role_name': dict(UserRole.choices).get(instance.role, instance.role),
            'description': f'{dict(UserRole.choices).get(instance.role, instance.role)}角色',
            'is_active': True,
            'sort_order': 0
        }
    )
    ...
    if not created:
        if role_user.role_level != role_level:
            ...
            role_user.save()
        if role_user.is_active != instance.is_active:
            role_user.is_active = instance.is_active
            role_user.save()
...
def cleanup_role_user_on_user_delete(sender, instance, **kwargs):
    """用户删除时清理RoleUser记录"""

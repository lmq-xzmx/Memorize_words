...
import type { RootState, UserState, LearningState, WordsState } from '../types/store'
import type { User, Word, LearningProgress, StudySession } from '../types'
import api from '../utils/api'

type ActionCtx<S = RootState> = ActionContext<S, RootState>
type UserActionCtx = ActionCtx<UserState>
type LearningActionCtx = ActionCtx<LearningState>
type WordsActionCtx = ActionCtx<WordsState>
...
const store = createStore<RootState>({
  ...
  mutations: {
    // 用户相关mutations
    SET_USER_AUTHENTICATED(state: RootState, status: boolean) {
      state.user.isAuthenticated = status
    },
    
    SET_USER_PROFILE(state: RootState, profile: User['profile']) {
      state.user.profile = profile
    },
    
    SET_USER_TOKEN(state: RootState, token: string | null) {
    ...
    // 学习相关mutations
    SET_CURRENT_WORD(state: RootState, word: Word | null) {
      state.learning.currentWord = word
    },
    
    SET_LEARNING_PROGRESS(state: RootState, progress: Record<string | number, LearningProgress>) {
      state.learning.learningProgress = progress
    },
    
    SET_STUDY_SESSION(state: RootState, session: StudySession | null) {
    ...
    SET_LANGUAGE(state: RootState, language: string) {
      state.app.language = language
      localStorage.setItem('language', language)
    },
    
    // 单词相关mutations
    SET_WORD_LIST(state: RootState, words: Word[]) {
      state.words.currentWordList = words
    },
    
    SET_SELECTED_WORDS(state: RootState, words: Word[]) {
      state.words.selectedWords = words
    },
    
    SET_SEARCH_RESULTS(state: RootState, results: Word[]) {
      state.words.searchResults = results
    },
    
    ADD_TO_FAVORITES(state: RootState, word: Word) {
      if (!state.words.favorites.find(w => w.id === word.id)) {
        state.words.favorites.push(word)
      }
    },
    
    REMOVE_FROM_FAVORITES(state: RootState, wordId: string | number) {
  ...
  actions: {
    // 用户认证actions
    async login({ commit }: ActionCtx, credentials: { username: string; password: string }) {
      try {
    ...
    async register({ commit }: ActionCtx, userData: any) {
      try {
        commit('SET_LOADING', true)
    ...
    // 获取用户信息
    async fetchUserProfile({ commit }: ActionCtx) {
      try {

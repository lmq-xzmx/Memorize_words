...
import { calculatePopupPosition, calculateSubmenuPosition, createPositionObserver } from '../utils/menuPositioning.js'
import { getCurrentUser, isAuthenticated } from '../utils/permission.js'
import { hasPermission, fetchUserMenuPermissions } from '../utils/dynamicPermission.js'
...
export function useMenuManager() {
  ...
  // 加载动态菜单权限
  const loadDynamicMenuPermissions = async () => {
    try {
  ...
  // 计算菜单位置
  const calculateMenuPosition = async (menuType, triggerRef, options = {}) => {
    await nextTick()
  ...
  // 计算子菜单位置 - 重构版本
  const calculateSubmenuPositionLocal = async (submenuType, parentMenuElement, options = {}) => {
    await nextTick()
  ...
  // 打开菜单
  const openMenu = async (menuType, triggerRef, options = {}) => {
    // 如果当前菜单已经是要打开的菜单，则关闭
  ...
  // 获取角色显示名称 - 统一使用roleDefinitions.js

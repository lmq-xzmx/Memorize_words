...
import { LEARNING_PERMISSIONS, CONTENT_PERMISSIONS, SOCIAL_PERMISSIONS } from './unifiedPermissionConstants.js'

// 学习模式页面路径与权限映射
export const LEARNING_MODE_PERMISSIONS = {
  // 基础学习模式
  '/word-learning': {
...
// 权限分类对应的页面
export const PERMISSION_CATEGORY_PAGES = {
  public: [],
  auth: [],
  user: [],
  learning: [],
...
Object.entries(PAGE_PERMISSIONS).forEach(([path, config]) => {
  const category = config.category || 'public'
 ...
 */
export function canAccessPage(userPermissions, path) {
 ...
 */
export function getAccessibleLearningModes(userPermissions) {
 ...
 */
export function getPagePermissionConfig(path) {
 ...
 */
export function pageRequiresAuth(path) {
 ...
 */
export function getAllAccessiblePages(userPermissions) {

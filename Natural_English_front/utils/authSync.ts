 ...
 */
export async function verifyBackendAuth(): Promise<boolean> {
  try {
...
/**
 * 从后端获取用户信息
 * @returns {Promise<UserInfo|null>} 用户信息或null
 */
export async function fetchUserFromBackend(): Promise<UserInfo | null> {
  try {
...
/**
 * 同步前后端登录状态
 * @returns {Promise<AuthSyncResult>} 同步结果
 */
export async function syncAuthState(): Promise<AuthSyncResult> {
  try {
    ...
    if (backendAuthenticated) {
      ...
      if (backendUser) {
        ...
        if (!frontendToken) {
          ...
          const tempToken = `temp_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
 ...
 * 在应用启动时调用
 */
export async function initAuthSync(): Promise<AuthSyncResult> {
 ...
 */
export function startAuthSyncInterval(interval: number = 5 * 60 * 1000): NodeJS.Timeout {
 ...
 * 在用户操作后调用
 */
export async function manualSyncAuth(): Promise<AuthSyncResult> {

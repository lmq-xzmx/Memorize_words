...
import { permissionSyncManager } from './permissionManager'
...
class WebSocketDiagnostics {
   ...
   * 处理WebSocket错误
   */
  private handleWebSocketError(source: string, message: string): void {
  ...
  private async checkNetworkConnection(): Promise<void> {
    ...
    try {
      const response = await fetch('http://localhost:8001/api/health-check', { 
        method: 'HEAD'
      })
   ...
   * 检查WebSocket配置
   */
  private checkWebSocketConfig(): void {
    const config: WebSocketConfig = {
      endpoint: importMeta.env.MODE === 'production' ? '/ws/permissions/' : 'ws://127.0.0.1:8001/ws/permissions/',
      retryAttempts: 5,
      retryDelay: 2000
    }
   ...
   * 检查后端服务状态
   */
  private async checkBackendService(): Promise<void> {
    const backendUrl = importMeta.env.MODE === 'production' ? '' : 'http://127.0.0.1:8001'
    ...
    return new Promise((resolve) => {
      ...
      try {
        ...
        testSocket.onerror = (error: Event) => {
          clearTimeout(timeout)
          this.addResult('error', 'WebSocket测试', `连接错误: ${(error as any).message || '未知错误'}`)
          resolve()
        }
   ...
   * 添加诊断结果
   */
  private addResult(type: DiagnosticResult['type'], category: string, message: string, details: any = null): void {

// 动态权限管理 - 从后端API获取权限数据
import { syncAuthState } from './authSync'
// 导入统一的API配置
import { getBackendHost, buildApiUrl, API_ENDPOINTS } from '../config/apiConfig'
 ...
 * 从后端获取用户菜单权限
 */
export async function fetchUserMenuPermissions(): Promise<UserMenuPermission[]> {
  try {
...
export function getRoleDisplayName(role) {
  ...
  // 临时兼容性映射，建议直接使用roleDefinitions.ts
  const roleNames = {
    'admin': '管理员',
    'dean': '教导主任',

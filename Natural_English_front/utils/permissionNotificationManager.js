...
class PermissionNotificationManager {
    ...
    setupWebSocketListeners() {
        webSocketManager.addListener('permissionChanged', (data) => {
            this.handlePermissionChange(data);
        });
        
        webSocketManager.addListener('roleUpdated', (data) => {
            this.handleRoleUpdate(data);
        });
        
        webSocketManager.addListener('menuAccessChanged', (data) => {
            this.handleMenuAccessChange(data);
        });
        
        webSocketManager.addListener('systemNotification', (data) => {
     ...
     * 处理菜单访问变更
     */
    async handleMenuAccessChange(data) {
    ...
    async showToastNotification(notification) {
        ...
        // 自动隐藏
        const autoHideDelay = this.config.autoHideDelay[notification.priority];
        if (autoHideDelay > 0) {
            setTimeout(() => {
     ...
     * 显示模态通知
     */
    async showModalNotification(notification) {
        const modalData = {
            id: notification.id,
     ...
     * 显示横幅通知
     */
    async showBannerNotification(notification) {
        const bannerData = {
            id: notification.id,
            message: notification.message,
            priority: notification.priority,
     ...
     * 显示徽章通知
     */
    async showBadgeNotification(notification) {
        // 更新通知徽章计数
     ...
     * 处理通知操作
     */
    async handleNotificationAction(notification, actionType) {
     ...
     * 播放通知效果
     */
    async playNotificationEffects(notification) {
        // 播放声音
     ...
     * 获取显示方法
     */
    getDisplayMethod(type, priority) {
     ...
     * 获取系统通知操作
     */
    getSystemNotificationActions(type) {
        if (type === 'maintenance') {
            return [
     ...
     * 格式化权限消息
     */
    formatPermissionMessage(resource, action) {
     ...
     * 格式化菜单变更消息
     */
    formatMenuChangeMessage(menuChanges) {
...
if (typeof window !== 'undefined') {
    window.permissionNotificationManager = permissionNotificationManager;
}

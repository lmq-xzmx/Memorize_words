...
class PermissionPerformanceMonitor {
     ...
     * 记录权限检查开始
     */
    startPermissionCheck(checkId, context = {}) {
     ...
     * 记录权限检查结束
     */
    endPermissionCheck(checkData, result = {}) {
     ...
     * 记录API请求
     */
    recordApiRequest(url, method = 'GET') {
     ...
     * 记录API响应
     */
    recordApiResponse(requestData, response = {}) {
     ...
     * 记录缓存操作
     */
    recordCacheOperation(operation, details = {}) {
     ...
     * 更新内存使用情况
     */
    updateMemoryUsage(cacheSize, memoryUsage) {
        if (!this.isMonitoring) return;
     ...
     * 检查性能警告
     */
    checkPerformanceAlerts(checkRecord) {
        const alerts = [];
     ...
     * 检查API性能警告
     */
    checkApiPerformanceAlerts(requestData) {

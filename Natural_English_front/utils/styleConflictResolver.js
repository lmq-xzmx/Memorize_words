...
class StyleConflictResolver {
   ...
   * 清理内联样式冲突
   */
  cleanInlineStyleConflicts() {
    const allElements = document.querySelectorAll('*[style]')
    
    allElements.forEach(element => {
      if (this.isNavigationElement(element)) return
      
      const style = element.getAttribute('style')
      if (this.hasConflictingStyles(style)) {
        this.resetElementStyle(element)
  ...
  generateConflictReport() {
    ...
    highZElements.forEach(element => {
      ...
      if (zIndex > 100 && !this.isNavigationElement(element)) {
...
if (typeof window !== 'undefined') {
  window.styleConflictResolver = styleConflictResolver
  window.resolveStyleConflicts = () => styleConflictResolver.resolveAllConflicts()
}

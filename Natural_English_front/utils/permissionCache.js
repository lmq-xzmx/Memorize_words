...
import { getRolePermissions } from './roleDefinitions.js'
...
class PermissionCacheManager {
  ...
  setupStorageListeners() {
    window.addEventListener('storage', (event) => {
      if (Object.values(CACHE_CONFIG.CACHE_KEYS).includes(event.key)) {
  ...
  isCacheExpired(key, timestamp) {
    ...
    const expiry = CACHE_CONFIG.CACHE_EXPIRY[key.toUpperCase()] || CACHE_CONFIG.CACHE_EXPIRY.USER_PERMISSIONS
  ...
  async getUserPermissions(userId, forceRefresh = false) {
    ...
    try {
      const result = await PermissionAPI.getUserPermissions()
      if (result.success) {
   ...
   * 获取角色权限（带缓存）
   */
  async getRolePermissions(role, forceRefresh = false) {
   ...
   * 添加到重试队列
   */
  addToRetryQueue(operation, params = {}) {
...
// 导出便捷函数
export async function getUserPermissions(userId, forceRefresh = false) {
  return await permissionCacheManager.getUserPermissions(userId, forceRefresh)
}

export async function getRolePermissionsFromCache(role, forceRefresh = false) {
...
export function startPermissionSync() {
  permissionCacheManager.startAutoSync()
}

export function stopPermissionSync() {
  permissionCacheManager.stopAutoSync()
}

export function addPermissionCacheListener(callback) {
  permissionCacheManager.addListener(callback)
}

export function removePermissionCacheListener(callback) {
...
export function cleanExpiredCache() {
  return permissionCacheManager.intelligentCacheEviction()
}
...
export function setPermissionCache(key, value, options = {}) {

...
declare global {
  interface Window {
    ...
    clearPersonalizationCache: () => void;
    forceRefresh: () => void;
    checkCacheStatus: () => CacheStatus;
  }
}
...
export class BrowserCacheManager {
   ...
   * @param prefix - 前缀
   */
  static clearStorageByPrefix(prefix: string): boolean {
    try {
   ...
   * 清除用户个性化相关的缓存
   */
  static clearPersonalizationCache(): void {
   ...
   * 检查是否有缓存数据
   */
  static checkCacheStatus(): CacheStatus {

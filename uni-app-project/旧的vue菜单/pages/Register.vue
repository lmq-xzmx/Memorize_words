...
export default {
  ...
  methods: {
    ...
    async loadRoleExtensions(role) {
      ...
      try {
        const response = await fetch(`http://127.0.0.1:8002/accounts/api/auth/role-extensions/?role=${role}`)
        if (response.ok) {
          const data = await response.json()
    ...
    async handleRegister() {
      ...
      try {
        ...
        if (this.roleExtensions.length > 0) {
          ...
          // 使用动态注册接口
          const response = await fetch('http://127.0.0.1:8002/accounts/api/auth/register-with-extensions/', {
            method: 'POST',
            headers: {

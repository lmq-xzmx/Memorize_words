...
export default {
  ...
  props: {
    ...
    showDevTools: {
      type: Boolean,
      default: () => import.meta.env.MODE === 'development' || false
    }
  },
  ...
  methods: {
    ...
    isMenuExpanded(menuId) {
      return menuStateManager.isMenuExpanded(menuId)
    },
    ...
    isDevToolEnabled(toolId) {
      return menuStateManager.isDevToolEnabled(toolId)
    },
    ...
    async handleMenuClick(menu) {
      ...
      try {
        // 设置活动菜单
        menuStateManager.setActiveMenu(menu.id)
    ...
    handleMenuToggle(menu) {
      menuStateManager.toggleMenuExpanded(menu.id)
    ...
    handleDevToolToggle(tool) {
      menuStateManager.toggleDevTool(tool.id)
    ...
    restoreActiveMenu() {
      ...
      if (currentPath) {
        menuStateManager.setActiveMenuByPath(currentPath)
      }
    },
    ...
    handleRouteChange(to) {
      menuStateManager.setActiveMenuByPath(to.path)
    }
  },

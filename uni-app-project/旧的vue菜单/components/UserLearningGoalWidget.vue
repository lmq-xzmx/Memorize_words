  ...
  methods: {
    ...
    async loadCurrentGoal() {
      try {
        ...
        // 调用学习目标API
        const response = await fetch('http://127.0.0.1:8002/teaching/api/learning-goals/', {
          headers: {
    ...
    goToLearningDashboard() {
      // 跳转到后端学习看板页面
      window.open('http://127.0.0.1:8002/teaching/learning-dashboard/', '_blank')
    },

    goToGoalManagement() {
      // 跳转到后端学习目标管理页面
      window.open('http://127.0.0.1:8002/teaching/goals/', '_blank')
    },

    createLearningGoal() {
      // 跳转到后端创建学习目标页面
      window.open('http://127.0.0.1:8002/admin/teaching/learninggoal/add/', '_blank')
    }
  }
}
</script>
